...
some_functions = make_array(min, print_kth_top_movie_year, num_non_vowels)
some_functions
...
def convert_percent_string_to_number(value):
    return float(value.strip("%"))

veteran_compensation = compensation.where("% Change", are.not_equal_to("(No previous year)"))
veteran_compensation = veteran_compensation.with_column(
    "% Change ($)",
    veteran_compensation.apply(convert_percent_string_to_number, "% Change")
)

with_previous_compensation = veteran_compensation.with_column(
    "2014 Total Pay ($)",
    veteran_compensation["Total Pay ($)"] / (1 + veteran_compensation["% Change ($)"]/100)
)
