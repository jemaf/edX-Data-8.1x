...
average_21st_century_rating = np.average(imdb.where('Year', are.above(1999))['Rating'])
...
even_year_movies = imdb.with_column('Year Remainder', imdb['Year'] % 2)

num_even_year_movies = even_year_movies.where('Year Remainder', are.equal_to(0)).num_rows
